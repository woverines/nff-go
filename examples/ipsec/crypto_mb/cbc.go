...
type MultiBlock interface {
	...
	// EncryptMany encrypts the first block in src into dst.
	// Dst and src may point at the same memory.
	EncryptMany(dst, src [][]byte)
...
func (this *cbcVectorEncrypter) CryptManyBlocks(dst, src [][]byte) {
	...
	if ok {
		...
		dst1 := make([][]byte, this.vecSize)
		src1 := make([][]byte, this.vecSize)
		for i := 0; i < this.vecSize; i++ {
			dst1[i] = dst[i]
			src1[i] = src[i]
		}
		for len(src1[0]) > 0 {
			...
			xorBytes(dst1, src1, iv, this.blockSize)
			this.mb.EncryptMany(dst1, dst1)
			for i := 0; i < this.vecSize; i++ {
				iv[i] = dst1[i][:this.blockSize]
				src1[i] = src1[i][this.blockSize:]
				dst1[i] = dst1[i][this.blockSize:]
			}
		}
	}
